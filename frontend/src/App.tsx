...
const PLAYBACK_EVENT_WINDOW_MS = 15 * 60 * 1000;

const LIVE_POSITION_POLL_MS = 5_000;
/*
...
function clipTrajectoryToPlaybackTime(
  ...
  if (trajectory === null || playbackTimeMs === null) {
    return trajectory;
  }

  const source = trajectory as unknown as PlaybackTrajectoryShape;
  ...
  const points = source.points.filter((point) => {
    const timestamp = new Date(point.timestamp).getTime();

    return Number.isFinite(timestamp) && timestamp <= playbackTimeMs;
  });
  ...
  const clipped = {
    ...
    start_time: points.length > 0 ? points[0].timestamp : null,

    end_time: points.length > 0 ? points[points.length - 1].timestamp : null,
...
function App() {
  const [collisionEncounters, setCollisionEncounters] = useState<
    CollisionEncounter[]
  >([]);

  const [selectedCollisionId, setSelectedCollisionId] = useState<number | null>(
    null,
  );
  ...
  const [selectedVesselCollisions, setSelectedVesselCollisions] = useState<
    CollisionEncounter[]
  >([]);
  ...
  const loadCollisionEncounters = useCallback(async (signal?: AbortSignal) => {
    try {
      setCollisionError(null);

      const response = await fetchCollisionEncounters(
        {
          limit: 500,
        },
        signal,
      ...
      setCollisionEncounters(response.items);
    } catch (caughtError) {
      if (signal?.aborted) {
        return;
      }

      console.error("Failed to load collision encounters:", caughtError);
   ...
   * must be completely disabled in that mode.
   */
  useEffect(() => {
    ...
    let collisionController: AbortController | null = null;
    ...
    const pollLiveData = async () => {
      ...
      collisionController = new AbortController();

      try {
        /*
         * Positions and collision overlays
         * can refresh independently.
         */
        const positionPromise = fetchRecentPositions(500);

        const collisionPromise = loadCollisionEncounters(
          collisionController.signal,
        );
         ...
         *
         * If playback started while the HTTP
         * request was running, never let the
         * completed live request overwrite the
         * historical playback frame.
         */
        if (!stopped) {
          setPositions(response.items);
        ...
        if (stopped || collisionController?.signal.aborted) {
          return;
        }

        console.error("Live position refresh failed:", caughtError);
    ...
    return () => {
      /*
       * Entering playback runs this cleanup.
       *
       * Any position response that finishes
       * afterward is ignored because stopped
       * is true.
       */
      stopped = true;
  ...
  }, [playbackTime, loadCollisionEncounters]);
  useEffect(() => {
    const controller = new AbortController();

    void loadCollisionEncounters(controller.signal);
  ...
  useEffect(() => {
    ...
    const loadSelectedVessel = async () => {
      ...
      try {
        const [trajectory, anomalyResponse, riskResponse, collisionResponse] =
          await Promise.all([
            ...
            fetchVesselCollisionEncounters(selectedMmsi, controller.signal),
          ]);
        ...
        setSelectedVesselCollisions(collisionResponse.items);
  ...
  const playbackTimeMs = useMemo(() => {
    if (playbackTime === null) {
      return null;
    }

    const value = new Date(playbackTime).getTime();

    return Number.isFinite(value) ? value : null;
  }, [playbackTime]);

  const isInsidePlaybackWindow = useCallback(
    (timestamp: string) => {
      ...
      return (
        eventTime <= playbackTimeMs &&
        eventTime >= playbackTimeMs - PLAYBACK_EVENT_WINDOW_MS
      );
    },
    [playbackTimeMs],
  );
   ...
   */
  const playbackTrajectory = useMemo(
    () => clipTrajectoryToPlaybackTime(selectedTrajectory, playbackTimeMs),
    [selectedTrajectory, playbackTimeMs],
  );
  ...
  const filteredAnomalies = useMemo(
    () =>
      selectedAnomalies.filter((anomaly) => {
        if (!isInsidePlaybackWindow(anomaly.observed_at)) {
          return false;
        }
  ...
  const filteredRisks = useMemo(
    () =>
      selectedRisks.filter((assessment) => {
        if (!isInsidePlaybackWindow(assessment.observed_at)) {
          return false;
        }
    ...
    [selectedRisks, riskLevelFilter, isInsidePlaybackWindow],
  );

  const playbackCollisionEncounters = useMemo(
    () =>
      collisionEncounters.filter((encounter) =>
        isInsidePlaybackWindow(encounter.observed_at),
      ),
    [collisionEncounters, isInsidePlaybackWindow],
  );

  const playbackVesselCollisions = useMemo(
    () =>
      selectedVesselCollisions.filter((encounter) =>
        isInsidePlaybackWindow(encounter.observed_at),
      ),
    [selectedVesselCollisions, isInsidePlaybackWindow],
  );

  const selectedAnomaly = useMemo(
    () =>
      filteredAnomalies.find((item) => item.id === selectedAnomalyId) ?? null,
    [filteredAnomalies, selectedAnomalyId],
  );

  const selectedRisk = useMemo(
    () => filteredRisks.find((item) => item.id === selectedRiskId) ?? null,
  ...
  const selectedCollision = useMemo(
    () =>
      playbackCollisionEncounters.find(
        (encounter) => encounter.id === selectedCollisionId,
      ) ?? null,
    [playbackCollisionEncounters, selectedCollisionId],
  );
  ...
  const elevatedRiskCount = useMemo(
    () =>
      filteredRisks.filter((assessment) => assessment.risk_level !== "low")
        .length,
    [filteredRisks],
  );
  ...
  const handleSelectCollision = useCallback(
    (collisionId: number) => {
      const encounter = playbackCollisionEncounters.find(
  ...
  }, [loadPositions, loadCollisionEncounters]);

  const handlePlaybackFrame = useCallback(
    (framePositions: RecentPosition[], requestedAt: string) => {
      setPlaybackTime(requestedAt);

      pendingRiskIdRef.current = null;

      setSelectedMmsi((currentMmsi) => {
        if (currentMmsi === null) {
          return null;
        }

        const stillVisible = framePositions.some(
          (position) => position.mmsi === currentMmsi,
        );

        return stillVisible ? currentMmsi : null;
      });
  ...
  return (
    <main className="dashboard">
      <header className="dashboard-header">
        <div>
          ...
          <p className="subtitle">
            Vessel monitoring, explainable AIS anomaly detection, hybrid
            investigation priority, and CPA/TCPA collision analysis.
          </p>
        </div>
      ...
      <LiveStatusBar historical={playbackTime !== null} />
      <section className="summary-grid" aria-label="Vessel position summary">
        ...
        <article className="summary-card">
          <span>Collision encounters</span>

          <strong>{playbackCollisionEncounters.length}</strong>
        </article>
      ...
      <CollisionQueue
        encounters={playbackCollisionEncounters}
      ...
      {collisionError !== null && (
        <div className="error-banner" role="alert">
          <strong>Could not load collision encounters.</strong>
      ...
      <section className="workspace">
        ...
        <section className="map-panel">
          <div className="map-panel-header">
            <div>
              ...
              <p>
                Click vessels, anomalies, risk markers, or collision encounter
          ...
          <VesselMap
            ...
            collisionEncounters={playbackCollisionEncounters}
        ...
        <aside className="details-panel">
          <div className="panel-heading">
            ...
            {(selectedPosition !== null || selectedCollision !== null) && (
              <button
                type="button"
          ...
          {selectedCollision !== null ? (
            ...
            <div className="empty-details">
              Select a vessel, collision encounter, anomaly, or risk observation
              to investigate.
            </div>
          ) : (
            <div className="vessel-details">
              ...
              <div className="selected-data-summary risk-summary-grid">
                {selectionLoading ? (
                  <span>
                    Loading trajectory, collisions, anomalies, and risk…
                  </span>
                ) : selectionError !== null ? (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <strong>{playbackVesselCollisions.length}</strong>
                    </div>
                    ...
                    <div>
                      ...
                      <strong>
                        {playbackTrajectory === null ? "Unavailable" : "Loaded"}
                      </strong>
                    </div>
                  </>
                )}
              ...
              <VesselCollisionList
                ...
                encounters={playbackVesselCollisions}
                    ...
                    <div className="risk-list">
                      ...
                      {filteredRisks.length === 0 && (
                        <div className="empty-risks">
                          No risk assessments match this playback frame and
                          filter.
                        </div>
                      )}
                    </div>
                    ...
                    <div className="anomaly-list">
                      ...
                      {filteredAnomalies.length === 0 && (
                        <div className="empty-anomalies">
                          No anomalies match this playback frame and filter.
                        </div>
                      )}
                    </div>
